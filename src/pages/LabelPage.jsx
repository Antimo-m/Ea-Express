...
function Label({ order }) {
  ...
  return <>
    ...
    <article className={`parcel-label document-${format}`}>
      ...
      <section className="label-destination"><small>DESTINATARIO / CONSEGNARE A</small><h2>{order.recipient_name}</h2><p>{order.delivery_address}<br/><strong>{order.delivery_city}</strong></p><p className="label-phone">Telefono {order.recipient_phone}</p></section>
      <section className="label-sender"><small>MITTENTE / RITIRARE DA</small><h3>{order.store_name}</h3><p>{order.pickup_address}<br/>{order.pickup_city}</p></section>
      <section className="label-facts"><div><small>RITIRO PROGRAMMATO</small><strong>{date(order.pickup_date)}</strong><span>{order.pickup_from} – {order.pickup_to}</span></div><div><small>PESO E DIMENSIONI</small><strong>{item ? `${item.weight_kg} kg` : 'Non specificati'}</strong>{item && <span>{item.length_cm} × {item.width_cm} × {item.height_cm} cm</span>}</div></section>
      {format === 'a4' && <section className="receipt-details"><h3>Informazioni di consegna</h3><dl><div><dt>Stato al momento della stampa</dt><dd>{order.status_label}</dd></div><div><dt>Corriere</dt><dd>{order.courier?.name || 'Da assegnare'}</dd></div><div><dt>Contenuto</dt><dd>{{other:'Altro',clothing:'Abbigliamento',documents:'Documenti'}[order.category]}</dd></div><div><dt>Valore merce dichiarato</dt><dd>{order.parcel_value_cents == null ? 'Non dichiarato' : money(order.parcel_value_cents)}</dd></div><div><dt>Costo spedizione</dt><dd>{order.price_cents == null ? 'Da concordare' : money(order.price_cents)}</dd></div></dl>{order.customer_notes && <div className="receipt-instructions"><h3>Istruzioni per il corriere</h3><p>{order.customer_notes}</p></div>}</section>}
